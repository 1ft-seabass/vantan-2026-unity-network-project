...
public class Week03_Chapter02_ClickPart_OK_Sample : MonoBehaviour, IPointerClickHandler
{
    ...
    IEnumerator RequestInit()
    {
        ...
        switch (request.result)
        {
            ...
            case UnityWebRequest.Result.Success:
                ...
                GameObject.Find("StatusMessage").GetComponent<TextMesh>().text = "あなたの pt は " + responseData.recordPoint.ToString() + " pt です！";

                // ポイント加算を反映
                addPoint = responseData.add_point;

                // 以前のポイントから再開
                currentPoint = responseData.recordPoint;

                GameObject.Find("CurrentPointMessage").GetComponent<TextMesh>().text = currentPoint.ToString() + " pt";
